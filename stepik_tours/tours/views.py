...
description = "Лучшие направления, где никто не будет вам мешать " \
              "сидеть на берегу и изучать программирование, дизайн, " \
              "разработку игр и управление продуктами"
departures = {"msk": "Из Москвы", "spb": "Из Петербурга",
              "nsk": "Из Новосибирска", "ekb": "Из Екатеринбурга",
              "kazan": "Из Казани"}
list_val = [title, subtitle, description, departures]

tours = {
    1: {
        "title": "Marina Lake Hotel & Spa",
        "description": "Отель выглядит уютно. Он был построен из "
                       "красного соснового дерева и украшен синими камнями."
    ...
    def get(self, request, *args, **kwargs):
        return render(
            request, 'tours/index.html', context={
                'tours': tours,
                'list_val': list_val,
                'ran': random_x(list(range(1, 17)), 6),  # рандомные отели в нижней части главной странице
                'cities': cities,
            }
        )
...
def minimax(tours_x, categ):
    minc, maxc = 1_000_000_000, -1
    for i in tours_x.values():
        if i[categ] > maxc:
            maxc = i[categ]
        if i[categ] < minc:
            minc = i[categ]
...
class DepartureView(View):
    def get(self, request, *args, **kwargs):
        # rp_city = request.path[-4:-1]  # костыль для получения названия города отправления
        rp_city = bad_re(request.path)
        tours_x = fun_tours_x(rp_city)  # dict туров для конкретного города
        prices = minimax(tours_x, "price")
        nights = minimax(tours_x, "nights")
...
random_tag_words = ['', 'комары размером со слона', 'сойдет', 'одни гуки', 'много водки', 'уже был',
                    'в Рязане круче', 'краcиво', 'супер', 'холодно', 'дорого', 'не рекомендую',
                    'так себе', 'жарко', 'тепло', 'сыро', 'незабываемо', 'ни за что на свете', 'кошмар',
                    'ниче так', 'волшебно', 'весело', 'скучно', 'вкусно', 'акулы', 'рекомендую']
    ...
    def get(self, request, *args, **kwargs):
        ...
        return render(request, 'tours/tour.html',
                      context={
                          ...
                          'list_val': list_val,
                          'rp_city': rp_city,
                          'tourx': tourx,  # тур по номеру из хвоста request.path
                          'tags1': random_x(random_tag_words, 3),  # рандомные тэги
                          'city': cities[tourx["departure"]]  # из какого города
                      }
                      )
